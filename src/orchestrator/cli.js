...
if (!['dry', 'run', 'resume'].includes(command)) fail('コマンドは dry / run / resume のいずれかです');
let input = values;
if (command === 'resume') {
...
try {
  const result = await createOrResume({ jobId: values['job-id'], prefecture: input.prefecture, area: input.area, category: input.category, dryRun: command === 'dry', stopBefore: values['stop-before'], headed: values.headed === true, maxItems: Number(values['max-items']) || undefined, maxPages: Number(values['max-pages']) || undefined });
