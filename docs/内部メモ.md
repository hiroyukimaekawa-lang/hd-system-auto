# HD Scraper Automation ― 内部メモ（管理者用）

> このファイルは仕組み・裏側の設定をまとめた管理者用の資料です。
> 日常のコムデスク投入の手順は、リポジトリ直下の `README.md` を参照してください。
>
> コムデスク自動投入だけを使う場合、以下の機能（Googleマップ/食べログ収集、
> 統合、Slack、AIアシスタント、GAS、夜間バッチ）は使いません。参考として残しています。
> これらのソースファイルは存在するだけでは動作を重くしません。重さの原因は
> リポジトリ直下で `npm install` したときに入る `better-sqlite3`（ネイティブビルド）・
> `@slack/bolt`・`playwright` です。コムデスク投入だけなら
> `comdesk-playwright-importer/` の中だけ `npm install` すれば足ります。
